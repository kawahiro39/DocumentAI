...
- `POST /document-ai/process/fields` : Bubble で扱いやすいキー形式の省略レスポンス
- `GET /health`
...
```json
{
  "fields": {
    "address": {
      "value": "タイムズ 中板橋第5",
      "confidence": 0.5980205
    },
    "invoice_number": {
      "value": "T4010001137274",
      "confidence": 0.92105585
    },
    "total_amount": {
      "value": "500",
      "confidence": 0.83346
...
- `fields` は配列ではなくオブジェクト形式なので、Bubble 側で `fields.address.value` のように参照しやすくしています。
- `document.entities` を汎用的に走査して生成するため、Processor 側でフィールドが追加・削除されても API コードの個別修正は不要です。
- 同名フィールドが複数存在する場合は、そのキーの値を配列にして返します。
- ネストした `properties` は `parent.child` 形式のキーで返します。
...
- 省略用パスは `document.entities` を元に動的なキー形式で `value` / `confidence` を返します。
